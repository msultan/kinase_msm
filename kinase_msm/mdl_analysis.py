...
class Project(object):
    def __init__(self,yaml_file):
        ...
        self.kmeans_mdl = verboseload(os.path.join(self.mdl_dir, "kmeans_mdl.pkl"))
...
class Protein(object):
    ...
    def __init__(self, series, name):
        ...
        self.bayes_mdl = verboseload('%s/bayesmsm_mdl.pkl'%self.protein_mdl_dir)
        self.tica_data = verboseload("%s/tica_data.pkl"%self.protein_mdl_dir)
        self.assignments = verboseload("%s/assignments.pkl"%self.protein_mdl_dir)
        self.fixed_assignments = verboseload("%s/fixed_assignments.pkl"%self.protein_mdl_dir)
    ...
    def _get_all_tics(self):
        ...
        if not self.tic_dict:
            ...
            for traj_index,traj_name in enumerate(self.fixed_assignments.keys()):
                ...
                for f_i,f in enumerate(self.fixed_assignments[traj_name]):
                    #go through and find the
                    try:
                        for tic_index in range(self.n_tics_):
                            self.tic_dict[tic_index][f].append(
