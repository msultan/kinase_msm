...
def create_assignment_matrix(assignments):
    ...
    :return: key_mapping: Dictionary that maps the array index to assignments
    file name
    :return: assignment_matrix: matrix of assignments of shape n_traj, length
     of max traj.
...
def create_tics_array(assignments, kmeans_mdl, tica_data):
    ...
    for index, trjname in enumerate(assignments.keys()):
        ...
        for tic_index in range(tics_to_use):
            tics_array[index, :current_traj_length, tic_index] = \
                tica_data[trjname][:, tic_index]
        key_mapping[index] = trjname
    return key_mapping, tics_array
